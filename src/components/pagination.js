...
export const initPagination = (
  ...
  pages.firstElementChild.remove();                             // и удаляем его (предполагаем, что там больше ничего, как вариант, можно и всё удалить из pages)
    ...
    const updatePagination = (total, { page, limit }) => {
        ...
        const from = total === 0 ? 0 : (page - 1) * limit + 1; // С какой строки выводим
        const to = Math.min(page * limit, total); // До какой строки выводим, если это последняя страница, то отображаем оставшееся количество
